...
| Page size | a paginated read answers **at most 100 rows per request**. A larger `limit` is **refused as invalid input rather than silently reduced**, so a screen cannot believe it has a complete list when it has a truncated one. 100 is comfortably above every screen this version describes — §11.2 and §12.2 each show one member of staff's own entries, and §3 foresees 50 members of staff — and far below a request that would read the table. `expenses` is the only paginated operation in 1.0.0; `monthlyExpenses` inherits the same ceiling. **A later screen needing a larger page is a change to this row, not a constant raised quietly** |
